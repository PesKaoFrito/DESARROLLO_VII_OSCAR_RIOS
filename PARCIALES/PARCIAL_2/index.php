...
switch ($action) {
    ...
    case 'edit':
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $datos = $_POST;
            $tarea = new Tarea($datos);
            if ($gestorTareas->actualizarTarea($tarea)) {
            ...
            $id = $_GET['id'] ?? '';
            $tareaEnEdicion = $gestorTareas->buscarTareasPorEstado($id)[0] ?? null;
        }
        break;
    ...
    case 'status':
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($gestorTareas->actualizarEstadoTarea($id, $nuevoEstado)) {
                $mensaje = "Estado de la tarea actualizado exitosamente.";
            } else {
    ...
    case 'filter':
        $estado = $_GET['estado'] ?? '';
        $tareas = $gestorTareas->buscarTareasPorEstado($estado);
        break;
